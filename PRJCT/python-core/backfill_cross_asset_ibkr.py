...
def _upsert_symbol_rows(db, symbol: str, provider: str, bars: Iterable) -> int:
    ...
    for bar in bars:
        ...
        db.cross_asset_candles.update_one(
            ...
            {"$set": {
                ...
                "timestamp": ts,
                "o": o, "h": h, "l": l, "c": c, "v": v,
                "source": "cross_asset_backfill_ibkr",
            }},
            upsert=True,
        )
    ...
    try:
        for sym in symbols:
            try:
                ...
                if contract is None:
                    print(f"[IBKR-BACKFILL] {sym}: ERROR no_qualified_contract")
                    fail += 1
                    continue
                sym_total = 0
                cursor_end = now
                while cursor_end > start:
                    ...
                    if not bars:
                        break
                    wrote = _upsert_symbol_rows(db, sym, "ibkr", bars)
                    sym_total += wrote
                    total += wrote
                ...
                print(f"[IBKR-BACKFILL] {sym}: ERROR {e}", flush=True)
                fail += 1
    finally:
        try:
            ib.disconnect()
